...
CORS_SET="[{\"allowedOrigins\":[\"${BASE_URL:-*}\"],\"allowedMethods\":[\"GET\",\"PUT\",\"HEAD\"],\"allowedHeaders\":[\"*\"],\"exposeHeaders\":[\"ETag\"],\"maxAgeSeconds\":3600}]"
...
if [ -n "${BUCKET_ID}" ]; then
  ...
  curl -sf -X POST "${ADMIN_URL}/v2/UpdateBucket?id=${BUCKET_ID}" \
    ...
    -d "{\"corsConfig\":{\"set\":${CORS_SET}}}" > /dev/null 2>&1 || echo "WARNING: CORS configuration failed"
else
